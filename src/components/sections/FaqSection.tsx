...
const faqs = [
  {
    q: "How does SafeTrace detect package damage?",
  ...
  {
    q: "Can logistics companies integrate SafeTrace?",
...
export default function FaqSection() {
  ...
  return (
    <section id="faqs" className="bg-surface-secondary px-4 py-20">
      <div className="mx-auto max-w-4xl">
        <p className="text-xs font-medium uppercase tracking-widest text-teal-400">FAQs</p>
        <h2 className="mt-3 text-3xl font-medium text-ink-primary">Everything people ask before shipping smarter</h2>
        <div className="mt-8 space-y-3">
          {faqs.map((item, idx) => (
            <div key={item.q} className="rounded-2xl border border-ink-muted/15  p-4 sm:p-5">
              <button
              ...
              >
                ...
                <ChevronDown
                  className={`h-4 w-4 text-ink-secondary transition-transform ${open === idx ? "rotate-180" : ""}`}
                />
              </button>
              {open === idx && <p className="mt-3 text-sm leading-relaxed text-ink-secondary">{item.a}</p>}
            </div>
          ))}
        </div>
      </div>
